...
export function SiteFooter(){
  const h={fontFamily:'var(--font-sans)',fontSize:'var(--t-eyebrow)',fontWeight:600,letterSpacing:'var(--ls-eyebrow)',textTransform:'uppercase',color:'var(--teal-300)',marginBottom:14};
  const a={fontFamily:'var(--font-sans)',fontSize:14,lineHeight:1.5,color:'rgba(243,246,251,.76)',textDecoration:'none'};
  return <footer style={{background:'var(--navy-900)',marginTop:0}}>
    <div style={{maxWidth:'var(--container)',margin:'0 auto',padding:'var(--sp-12) 24px var(--sp-8)',display:'grid',gridTemplateColumns:'1.3fr repeat(4,1fr)',gap:'var(--sp-8)'}}>
      <div style={{display:'grid',gap:16,alignContent:'start'}}>
        <span style={{background:'#fff',borderRadius:'var(--r-md)',padding:'8px 12px',display:'inline-flex',width:'fit-content'}}><img src="../../assets/sinu-logo.jpg" alt="SINU" style={{height:36}}/></span>
        <p style={{margin:0,fontFamily:'var(--font-sans)',fontSize:13.5,lineHeight:1.65,color:'rgba(243,246,251,.6)',maxWidth:280}}>Kukum Campus, Honiara, Solomon Islands<br/>(+677) 42600</p>
        <div style={{display:'flex',gap:10}}>
          {SOCIAL.map(([label,d])=>
            <a key={label} href="#" aria-label={label} title={label} style={{width:36,height:36,borderRadius:'50%',border:'1px solid rgba(243,246,251,.24)',display:'inline-flex',alignItems:'center',justifyContent:'center',color:'rgba(243,246,251,.8)'}}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><path d={d}/></svg>
            </a>)}
        </div>
      </div>
      {COLS.map(([title,items])=>
        <div key={title} style={{display:'grid',gap:9,alignContent:'start'}}>
          ...
          {items.map(i=><a key={i} style={a} href="#">{i}</a>)}
        </div>)}
    </div>
    <div style={{borderTop:'1px solid rgba(243,246,251,.14)'}}>
      <div style={{maxWidth:'var(--container)',margin:'0 auto',padding:'14px 24px',display:'flex',gap:20,flexWrap:'wrap',fontFamily:'var(--font-sans)',fontSize:12.5,color:'rgba(243,246,251,.5)'}}>
        <span>© 2026 Solomon Islands National University. All rights reserved.</span>
        <span style={{flex:1}}/>
