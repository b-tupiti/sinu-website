...
export async function cmsFetch<T>(
  ...
  if (json.errors) {
    // GraphQL allows partial success: field-level errors come back
    // alongside otherwise-usable data. Warn (not error) so it doesn't
    // trip Next's dev overlay, but fail the request when there's no
    // data left.
