...
/**
 * Wagtail's admin preview button lands here with a one-shot ?token=.
 * We resolve it to a real page URL, enable draftMode, and redirect the
 * editor to that URL with the token preserved so the catch-all route
 * fetches the unsaved revision via GET_PAGE_BY_TOKEN.
 */
export async function GET(request: Request) {
  try {
    ...
    const data = await cmsFetch<{ page: PageMeta | null }>(
      GET_PAGE_META_BY_TOKEN,
      { token: previewToken },
    );
    ...
    return new Response("Internal Server Error inside Preview Route Handler", {
      status: 500,
    });
  }
}
